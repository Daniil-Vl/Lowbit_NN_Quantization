...
def __quantize_tensor(tensor: torch.Tensor, scale: int, zero_point: int, device: str = 'cpu') -> QuantizedTensor:
    ...
    # FIXME: Was cuda
    return torch.round(res/scale) - torch.tensor([zero_point]).to(device)
...
# FIXME:
# TODO: Implement here symmetric quantization
def quantize(tensor: torch.Tensor, bitwidth: int = 8, train: bool = True, symmetric: bool = False, device='cpu') -> QuantizedTensor:
    ...
    """
    # Plan
    # 1) Calculate clipping range(a, b)
    # 2) Calculate scale
...
# TODO: Create global observer
# It will be saving scales and zero-point for activations map

# Quantization Aware Training ====================================================================
class FakeQuantOp(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, num_bits=8, device='cpu', min_val=None, max_val=None):
        # x = quantize(x,num_bits=num_bits, min_val=min_val, max_val=max_val)
...
# FIXME: TODO: Improve performance speed
# FIXME: TODO: Change device to cpu by default
# FIXME: TODO: Do something with batchnorm
...
# TODO: TO implement
def quantize_model(model: torch.nn.Module, bit_width: int = 8):
    """
    Quantize weights using min-max range
    """
    pass


# FIXME: Now this is not only int computationals, because features can be float, and they are in range 0...1
